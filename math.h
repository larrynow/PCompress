...
namespace NCMath
{
	...
	struct VEC3
	{
		...
		float SqrDistance(const VEC3& v) const
		{
	...
	struct MAT4
	{
		...
		inline void SetRow(std::size_t row, const float(&newRows)[4])
		{
			if (row < 4)
			{
		...
		inline VEC4 GetRow(std::size_t row) const
		{
			return VEC4(m[row][0], m[row][1], m[row][2], m[row][3]);
		}

		inline void SetColumn(std::size_t col, const float(&newCols)[4])
		{
			if (col < 4)
			{
				for (size_t i = 0; i < 4; i++)
		...
		inline void SetColumn(std::size_t col, const VEC4& newCol)
		{
			if (col < 4)
			{
				for (size_t i = 0; i < 4; i++)
		...
		inline VEC4 GetColumn(std::size_t col) const
		{
	...
	inline float Clamp(float val, std::size_t min, std::size_t max) { return Clamp(val, float(min), float(max)); };
